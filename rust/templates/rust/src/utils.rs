...
pub async fn send_notification(greeting_id: &str, name: &str) -> Result<(), HandlerError> {
    if random::<f32>() < 0.5 {
        ...
        return Err(HandlerError::from(anyhow!("[SIMULATED RANDOM FAILURE] Failed to send notification: {} - {}", greeting_id, name)));
    }
...
pub async fn send_reminder(greeting_id: &str)-> Result<(), HandlerError> {
    if random::<f32>() < 0.5 {
        ...
        return Err(HandlerError::from(anyhow!("[SIMULATED RANDOM FAILURE] Failed to send reminder: {}", greeting_id)));
    }
